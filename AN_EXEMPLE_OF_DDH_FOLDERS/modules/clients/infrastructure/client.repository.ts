...
export class ClientRepository implements ClientPort{
    ...
    public async getClientInformationById(clientId: string): Promise<ClientEntity> {
        const clientOrmEntity = await this.clientRepository.findOne({
            where: { id:clientId },
          });
        if(!clientOrmEntity){
            throw new NotFoundException(`client with id ${clientId} does not exist in database`)
        }
