...
import { GetClientInformationPresenter } 
    from "../../client/interface/get-client-information.presenter";
import { PizzaCommandSentEvent } 
    from "../../pizza/domain/pizza_command/events/pizza-command-sent.event";
import { 
...
export class PizzaCommandSentEventHandler implements IEventHandler<PizzaCommandSentEvent>{
    ...
    async handle( { payload }:PizzaCommandSentEvent ) : Promise<void> {
        ...
        const message = 
        `Your pizzas are supposed to be delivered at : ${plannedDeliveryDate}`;

        await this.commandBus.execute<SendMessageToClientCommand,
         SendMessageToClientCommandResult>(
